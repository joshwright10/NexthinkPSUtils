Describe "Get-RelatedRemoteAction" {
    BeforeAll {
    ...
    It "should resolve to a single Remote Action" {
        ...
        $remoteActionsToCheck = [System.Collections.Generic.List[System.Xml.XmlElement]]::new()
        $xmlContent.SelectNodes("//Action/Schedule") | ForEach-Object { $remoteActionsToCheck.Add($_) }

        $result = Get-RelatedRemoteAction -RemoteActions $remoteActionsToCheck
    ...
    It "should resolve multiple Remote Actions" {
        ...
        $results = Get-RelatedRemoteAction -RemoteActions $remoteActionsToCheck
